const AVAILABLE_WEEK_DAYS = ['Domingo', 
                            'Lunes', 
                            'Martes', 
                            'Miercoles', 
                            'Jueves', 
...
class CALENDAR {
    constructor(options) {
        ...
        //mostrar los eventos guardados en local
...
// App methods
    ...
    // listar los eventos guardados
    drawEvents() {
        let calendar = this.getCalendar();
        let eventList = this.eventList[calendar.active.formatted] || ['No hay actividades'];
        let eventTemplate = "";
    ...
    drawDays() {
        ...
        let daysInActiveMonth = this.range(calendar.active.days).map((day, idx) => {
            ...
            return {
                dayNumber,
                today: today.getDate() === dayNumber && today.getFullYear() === calendar.active.year && 
        ...
        let days = [...latestDaysInPrevMonth, ...daysInActiveMonth, ...daysInNextMonth];
         ...
         //---- diseño del dia para eventos
        days = days.map(day => {
            let newDayParams = day;
            let formatted = this.getFormattedDate(new Date(`${Number(day.month) + 1}/
            ${day.dayNumber}/${day.year}`));
            newDayParams.hasEvent = this.eventList[formatted];
            return newDayParams;
        });
        //-----------------------------------

        //eventos de los dias cuadro derecho
        let daysTemplate = "";
        days.forEach(day => {
            daysTemplate += `<li class="${day.currentMonth ? 
                '' : 'another-month'}${day.today ? 
                    ' active-day ' : ''}${day.selected ? 
                        'selected-day' : ''}${day.hasEvent ? 
                            /*muestrar el punto del evento agregado*/
                            ' event-day' : ''}" 
                            data-day="${day.dayNumber}" 
                            data-month="${day.month}" 
                            data-year="${day.year}"></li>`
        });
    ...
    drawMonths() {
        ...
        availableMonths.forEach((month, idx) => {
            monthTemplate += 
            `<li class="${idx === calendar.active.month ? 
                'active' : ''}" data-month="${idx}">${month}</li>`
        });
    ...
    eventsTrigger() {
        ...
        this.elements.eventAddBtn.addEventListener('click', e => {
            ...
            // guardo el dato en el localStorageName
...
function agregarEvento() {
    var x = document.getElementById("eventoNuevo");
